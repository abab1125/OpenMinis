...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /**
         * Novel-writing + Hermes backend: two additive nullable columns on
         * sessions. `book_id` binds a session to a book project so BookTools
         * are injected even for ordinary (non-bookshelf) chats; `backend`
         * marks a session as a transparent Hermes gateway passthrough (local
         * agent loop bypassed). Both default NULL = existing behaviour.
         */
        val MIGRATION_10_11 = object : Migration(10, 11) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE sessions ADD COLUMN book_id TEXT")
                db.execSQL("ALTER TABLE sessions ADD COLUMN backend TEXT")
            }
        }
